...
### 1⃣ `list-kafka-topics.sh` - Kafka Topics 조회
...
### 2⃣ `insert-test-data.sh` - 테스트 데이터 관리
...
### 3⃣ `view-cdc-messages.sh` - CDC 메시지 조회
...
### 4⃣ `check-connector-status.sh` - Connector 상태 확인
...
### 5⃣ `check-binlog-status.sh` - Binlog 상태 확인
...
### 6⃣ `cdc-test-flow.sh` - 전체 플로우 테스트

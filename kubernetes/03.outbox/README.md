...
│
├── script/
│   ├── 00.create-outbox-event.sh  # outbox_events 테이블 생성
│   ├── 01.insert-outbox-event.sh  # 테스트 이벤트 직접 삽입 (DB 접근)
│   ├── 02.check-kafka-topic.sh    # Kafka Topic 메시지 확인
│   └── 03.test-consumer.sh        # Consumer API 응답 확인
│
└── sql/

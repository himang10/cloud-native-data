...
if [ -z "${CONNECTOR_FILE}" ]; then
    ...
    echo ""
    echo "예제:"
    echo "  $0 mariadb-outbox-connector.json"
    echo "  $0 mariadb-outbox-cloudevents-connector.json"
    exit 1
fi
...
if [ $? -eq 0 ]; then
    ...
    if [ "${CONNECTOR_STATE}" = "RUNNING" ] && [ "${TASK_STATE}" = "RUNNING" ]; then
        ...
        echo ""
        echo "Outbox 이벤트 라우팅:"
        if [[ "${CONNECTOR_NAME}" == *"cloudevents"* ]]; then
            echo "  • aggregate_type='user' → cloudevents.user"
            echo "  • aggregate_type='order' → cloudevents.order"
            echo "  • aggregate_type='product' → cloudevents.product"
        else
            echo "  • aggregate_type='user' → outbox.user"
            echo "  • aggregate_type='order' → outbox.order"
            echo "  • aggregate_type='product' → outbox.product"
        fi
    else
    ...
    echo "  이벤트 확인: ./view-outbox-events.sh <topic-name>"
else
    echo "❌ 오류: 커넥터 등록 실패"
    exit 1
fi
